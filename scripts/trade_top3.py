...
def main() -> int:
    ...
    for _, row in df.iterrows():
        ...
        jlog('trade_top3_submit', symbol=sym, status=str(rec.status), qty=int(qty), price=float(rec.limit_price))
        print(f'{sym} -> {rec.status} @ {rec.limit_price} qty {qty} note={rec.notes}')
        submitted += 1

    print(f'Submitted {submitted} IOC LIMIT order(s).')
    return 0
