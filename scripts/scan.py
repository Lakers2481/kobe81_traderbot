...
Features:
- Loads universe from data/universe/optionable_liquid_800.csv (800 symbols)
...
def load_optimal_pattern_config() -> Dict:
    """
    Load optimal pattern configuration from state/optimal_pattern.json.

    This config is generated by scripts/optimize_consecutive_pattern.py
    which tests MULTIPLE patterns (2-10 days) and finds the optimal one
    using professional quant methodology (Bonferroni correction, DSR).

    Returns:
        Dict with optimal_streak_length (default 5 for backwards compat)
    """
    config_path = ROOT / "state" / "optimal_pattern.json"

    if not config_path.exists():
        # Backwards compatible: default to 5 days if not optimized yet
        return {
            "optimal_streak_length": 5,
            "source": "default",
            "note": "Run scripts/optimize_consecutive_pattern.py to find data-driven optimal"
        }
    ...
    try:
        ...
        streak_length = config.get("streak_length", 5)

        return {
            "optimal_streak_length": streak_length,
            "source": "optimized",
            "verified_date": config.get("verified_date"),
            "win_rate": config.get("win_rate"),
            "p_value": config.get("p_value"),
            "is_significant": config.get("is_significant", False),
        }
    except Exception as e:
        print(f"⚠ Warning: Failed to load optimal pattern config: {e}")
        return {
            "optimal_streak_length": 5,
...
Examples:
  python scripts/scan.py --cap 800 --deterministic --top5     # Full pipeline
  ...
  python scripts/scan.py --json                               # Output as JSON
    ...
    # === LOAD OPTIMAL PATTERN CONFIGURATION ===
    # Uses data-driven optimal streak length from pattern optimization
    # Falls back to 5 days if optimization hasn't been run yet
    optimal_pattern_config = load_optimal_pattern_config()
    OPTIMAL_STREAK_LENGTH = optimal_pattern_config["optimal_streak_length"]

    if args.verbose:
        source = optimal_pattern_config.get("source", "unknown")
        if source == "optimized":
            print(f"✓ Using optimized pattern: {OPTIMAL_STREAK_LENGTH} consecutive days")
            print(f"  Verified: {optimal_pattern_config.get('verified_date', 'unknown')}")
            print(f"  Win Rate: {optimal_pattern_config.get('win_rate', 0):.1%}")
            print(f"  Significant: {optimal_pattern_config.get('is_significant', False)}")
        else:
            print(f"ℹ Using default pattern: {OPTIMAL_STREAK_LENGTH} consecutive days (not optimized)")
            print(f"  Run scripts/optimize_consecutive_pattern.py to find data-driven optimal")
    ...
    if args.json:
        ...
        if not args.top3:
            ...
            if not signals.empty:
                ...
                # === HISTORICAL PATTERN BOOST + AUTO-PASS ===
                # Boost confidence for strong historical patterns
                # AUTO-PASS: Elite patterns (20+ samples, 90% WR, 5+ days) = 95% score
                if 'streak_length' in df.columns or 'qualifies_auto_pass' in df.columns:
                    try:
                        def apply_pattern_boost(row: pd.Series) -> float:
                            ...
                            # AUTO-PASS: Elite historical patterns bypass everything
                            if row.get('qualifies_auto_pass', False):
                                return 0.95  # ELITE tier - guaranteed Top 2

                            # Strong historical pattern (OPTIMAL days, 80%+ WR)
                            streak_length = int(row.get('streak_length', 0))
                            streak_win_rate = float(row.get('streak_win_rate', 0.0))

                            if streak_length >= OPTIMAL_STREAK_LENGTH and streak_win_rate >= 0.80:
                                return max(0.0, min(0.95, base + 0.10))  # +10% boost

                            # Moderate historical pattern (OPTIMAL days, any WR)
                            if streak_length >= OPTIMAL_STREAK_LENGTH:
                                return max(0.0, min(0.95, base + 0.05))  # +5% boost

                            return base  # No pattern boost

                        df['conf_score'] = df.apply(apply_pattern_boost, axis=1)
                        if args.verbose:
                            auto_pass = df[df.get('qualifies_auto_pass', pd.Series([False] * len(df))) == True]
                            strong_pattern = df[(df.get('streak_length', pd.Series([0] * len(df))) >= OPTIMAL_STREAK_LENGTH) &
                                              (df.get('streak_win_rate', pd.Series([0.0] * len(df))) >= 0.80)]
                            if len(auto_pass) > 0:
                                print(f"[AUTO-PASS] {len(auto_pass)} signals with elite patterns (score = 95%)")
                            if len(strong_pattern) > 0:
                                print(f"[PATTERN BOOST] {len(strong_pattern)} signals with strong patterns (+10%)")
                    except Exception as e:
                        if args.verbose:
                            print(f"[PATTERN BOOST] Failed: {e}")
                ...
                if args.top5 and not df.empty:
                    ...
                    if args.verbose:
                        print(f"\n[KOBE PIPELINE] 800 -> 5: Filtered to top {len(top5_df)} candidates")
            ...
            if out.empty:
                ...
                # === KOBE STANDARD PIPELINE OUTPUT: 800 -> 5 -> 2 ===
                print(f"\n{'='*70}")
                print(f"KOBE PIPELINE: 800 -> 5 (STUDY) -> 2 (TRADE) | DATE: {signal_date}")
                print(f"{'='*70}")
    ...
    if not signals.empty:
        ...
        # Final summary
        print(f"\n{'='*70}")
        print("KOBE PIPELINE: 800 STOCKS + OPTIONS + CRYPTO -> TOP 5 -> TOP 2")
        print("="*70)
