...
def generate_ticker_profile(
    ...
    lines.append(f"**Recovery:** 7 trading days")
    lines.append("")
    ...
    if len(streak_events) > 0 and 'days_to_recover_close' in streak_events.columns:
        ...
        recovered = streak_events[streak_events['recovered_7d_close'] == True]
    ...
    lines.append("")
    lines.append("---")
    lines.append(f"*Generated by Kobe Bounce Analysis System*")
...
def generate_summary_report(
    ...
    lines.append(f"**Recovery:** 7 trading day forward window")
    lines.append("")
    ...
    if per_stock_df is not None and len(per_stock_df) > 0:
        ...
        lines.append(f"| Quality Flag | Count |")
        lines.append(f"|--------------|-------|")
